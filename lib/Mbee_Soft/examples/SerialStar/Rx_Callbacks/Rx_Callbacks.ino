...
/** Скетч предназначен для демонстрации работы с библиотекой с помощью callback функций. 
  ...
  * Для контроля приема данных, к используется светодиод, установленный на плату Arduino.
  * 
...
void setup() 
{
  ...
  mbee.onRxResponse(rxResponseCallback); //Регистрируем функцию, которая будет автоматически вызываться каждый раз после приема пакета с неструктурированными данными от удаленного узла.
